...
public class ValidarIdentificacion {
     ...
     * @throws Exception
     */
    public boolean validarCedula(String numeroCedula) throws Exception {
        try {
            validarInicial(numeroCedula, 10);
            validarCodigoProvincia(numeroCedula.substring(0, 2));
            validarTercerDigito(String.valueOf(numeroCedula.charAt(2)), TipoDocumento.getTipoCedula());
            algoritmoModulo10(numeroCedula, Integer.parseInt(String.valueOf(numeroCedula.charAt(9))));
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
